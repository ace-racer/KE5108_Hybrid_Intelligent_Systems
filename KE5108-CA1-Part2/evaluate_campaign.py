...
def get_best_customers_total_score(df, output_name = None):
    ...
    if df is not None:
        df["status_multiplier"] = df["status"].replace("A", 0.6).replace("B", 1).replace("None", 0)
        df["expected_profit"] = df["status_multiplier"] * df["score"]
        df = df.sort_values("expected_profit", ascending=False)

        # get top customer
        top_customer_df = df.head(NUM_CUSTOMERS_TO_CHOOSE)
        
        if output_name:
            output_file_location = os.path.join("results", output_name + ".csv")
            top_customer_df.to_csv(output_file_location, index=False)
            print("Generated the output file here: {0}".format(output_file_location))

        total_expected_profit = np.sum(top_customer_df.loc[:, ["expected_profit"]])
...
# python evaluate_campaign.py "H:\\KE 5108 - CAs\\code\\KE 5108 -CA1-Part 2\\results\\cust_actual_predicted.csv" cust_actual_merged_best

if __name__=="__main__":
    if len(sys.argv) < 2:
        print("Usage: python evaluate_campaign.py \"H:\\KE 5108 - CAs\\code\\KE 5108 -CA1-Part 2\\results\\cust_actual_predicted.csv\" cust_actual_merged_best")
    else:
